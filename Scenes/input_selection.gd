...
const SCHEME_AB_KEYS := {
	Scheme.WASD: [KEY_J, KEY_K],
	Scheme.ARROWS: [KEY_Z, KEY_X],
}
...
func _try_confirm() -> void:
	...
	_transition_to_scene("res://Scenes/game_scene.tscn")

func _apply_scheme(scheme: Scheme) -> void:
	for action in SCHEME_BINDINGS[scheme]:
		InputMap.action_erase_events(action)
		var ev := InputEventKey.new()
		ev.physical_keycode = SCHEME_BINDINGS[scheme][action]
...
func _update_visuals() -> void:
	...
	wasd_option.modulate = Color(1, 1, 1, lerpf(0.35, 1.0, wasd_progress))
	arrows_option.modulate = Color(1, 1, 1, lerpf(0.35, 1.0, arrows_progress))
